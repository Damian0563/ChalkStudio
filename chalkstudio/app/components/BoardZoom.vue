	...
	<div
		...
		<div class="flex items-stretch">
			<button type="button"
				...
				aria-label="Zoom out" title="Zoom out" @click="decreaseZoom">
				<Icon name="lucide:minus" class="h-3.5 w-3.5" aria-hidden="true" />
			</button>
			<div class="flex min-w-[5rem] flex-col items-center justify-center gap-0.5 border-x border-chalk/10 px-3 py-1.5">
				<span class="font-display text-[0.9375rem] font-semibold tabular-nums leading-none tracking-tight text-chalk">
					{{ zoomPercent }}%
				</span>
			</div>
			<button type="button"
				class="flex h-10 w-10 shrink-0 items-center justify-center text-chalk-faint transition-colors hover:bg-chalk/[0.06] hover:text-chalk active:bg-coral/15 active:text-coral-soft"
				aria-label="Zoom in" title="Zoom in" @click="increaseZoom">
